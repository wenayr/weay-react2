...
export function Card52() {
    return (
        <Check id="floating-window-stack" n={52} title="FloatingWindow - desktop taskbar, sessions, cascade and Snap"
               do="Open both windows. A has a title-bar minimize button; B intentionally hides it—focus B and use Win/Meta+Down instead. Restore either window from the common bottom panel. Double-click/tap a title twice to maximize and restore: there is intentionally no maximize button. Then maximize one and drag it away by its title bar. Drag to the top centre and inspect all three Snap presets; the last must be a complete 2x2 with four clickable corners. Reload after snapping to confirm the group layout returns."
               expect="Only one visible window is active. Exactly one title-bar Minimize button is rendered, while both windows remain minimizable through external surfaces. Restoring raises the whole isolated layer including scoped popups. Dragging a maximized or snapped window off the edge returns it to the size it had before, under the pointer, while a press that does not travel leaves it alone so double-click still toggles. The Snap picker contains 2, 2 and 4 zones—columns, rows, quarters—never two detached bottom cells. Snap/free geometry returns after reload; new windows cascade."

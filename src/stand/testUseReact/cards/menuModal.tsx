...
export function Card4() {
    ...
    <Check n={4} title="Right-click context menu (contextMenu)"
                       ...
                       note="Fix: the shared useContextMenuGesture stores an item snapshot on open + menuMouse onConsume. Menu items now use explicit actionKey for local action stats."
                       tall>
...
/* ---------- 54. createRightMenuController dropdown (demo moved out of the library) ---------- */
export function Card54() {
    return (
        <Check n={54} title="RightMenu dropdown (createRightMenuController)"
               do="Hover the three items; open each submenu and drag the outline box inside Item 1."
               expect="Each item opens its submenu on hover; submenus stay open while the pointer is inside and the drag demo inside Item 1 still drags."
               note="2.0.0: RightMenuDemo/SubMenu/SubMenu2 no longer ship from wenay-react2 - they live in src/stand/testUseReact/rightMenuDemo.tsx. The library exports only createRightMenuController."
               tall>
            <div style={{ height: 300 }}><RightMenuDemo/></div>
        </Check>
    );
}

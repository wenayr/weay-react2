...
function triggerUpdate(obj: object, reverse = false, lastOnly = false) {
    let state = map3.get(obj);
    const idle = !state || (state.listeners.size === 0 && state.callbacks.size === 0);
...
export function createUpdateApi<T extends object>(obj: T): UpdateApi<T> {
    ...
    const api: UpdateApi<T> = {
        ...
        // minus the per-callback try/catch, so it now shares `state.callbacks` with
        // useUpdateBy(a, f): imperative subscribers still run first, and reverse/lastOnly still
        // apply to the React notifiers only.
        on(listener: (obj: T) => void) {
            const state = getObserverState(obj);
            const wrapper: Listener = (value) => { listener(value as T); };
            state.callbacks.add(wrapper);
            return () => { state.callbacks.delete(wrapper); };
        },

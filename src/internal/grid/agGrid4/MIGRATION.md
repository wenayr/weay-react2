...
|---|----------|-------|-------------------|----------|--------|
| 1 | `pages/clientPage/main.tsx:866` | 1 (main table) | applyGridRows x6, streams | yes | `datum.tableArr` (module) |
...
10. **Theme**: production grids do NOT pass theme (v35 gives default Quartz / legacy CSS through GridStyleDefault from wenay-react2, imported in src/index.ts). AgGridTable sets Alpine dark, so **the appearance will change**. Decide in the pilot: either accept Alpine or pass through the current theme.
...
- Smoke test on the sandbox `pageTest3.tsx` (throwaway static grid): `<AgGridTable data=...>`; check theme, resize, and selection live.
- Decide the theme question here (defect 10): compare AgGridTable against the current production grid.
...
- Grid :870 -> `<AgGridTable controller>`; keep rowData-useState (:238) as initial state, then remove it in favor of the buffer in a second iteration.
...
- Bare AgGridReact -> `<AgGridTable controller>`: keep existing onGridReady side effects (filter model, tableReady, EmptyColumn, renderBy), chaining them after the wiring; remove manual sync and manual ref clearing (lifecycle is in the wrapper).
...
- Last consumer of v1 from wenay-react2 -> `useAgGrid({ getId, externalBuffer: object.current })`; 2 calls -> `update`. Grid -> AgGridTable. This removes the v1 dependency entirely.

### Stage 4. Exchange Modal Grids [Complexity L, Risk L]
- binance.tsx History Transfer: -> controller + AgGridTable; fix keys (defect 2): a single `getId: e=>String(e.tranId)` (stable natural key) instead of timestamp/`a++`.
- gateio.tsx loans: declarative `<AgGridTable data={getRowData()}>` or just the AgGridTable wrapper; clear refs through the controller (currently not cleared).
...
- Everywhere: `<AgGridTable data={rows} columnDefs=...>` (or controller where imperative access exists); add getRowId where missing (defect 3); remove custom ResizeObserver/sizeColumnsToFit.
...
- Remove local v1 in selectSymbol (stage 1), and imports of `applyGridRows` from wenay-react2.
...
|------|-------|------------|
| Appearance change (Alpine dark vs current theme) | all grids | Stage 0: sandbox comparison; AgGridTable accepts `theme` |

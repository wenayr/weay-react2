...
`/windows`, `/logs`, `/params`, `/modal`, `/menu`, `/chart`, `/ui`; `/native`
...
- **Auth renewal stays transport-owned.** With `wenay-common2@2.2.x`, prefer
  the hub `token` provider plus `authListen` for application auth state instead
  of remounting React trees or adding socket listeners. Server expiry,
  revocation, and a narrowed principal end exactly the Listen streams that are
  no longer authorized; existing React subscriptions observe that normal end
  and retain their usual idempotent cleanup contract. The application still
  owns credentials, login UI, renewal policy, and creation of a new authorized
  subscription when access is granted again.
...
- Since 5.0.0 the React adapters named above (`usePeer`, `usePeerCalls`, `usePeerPresence`, `useMediaSource`, `useRouteState`) and `VideoCall` live in the separate `wenay-calls` package (`packages/wenay-calls`).

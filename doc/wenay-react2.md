...
// Store Replay V2 (Observe.exposeStoreReplay(...).api.replay)
const mirror = useStoreReplayMirror(remote, initial, {enabled?, onBatch?, validateBatch?}) // creates the mirror store; same staleMs/stale/policy surface
mirror.store; mirror.ready; mirror.seq(); mirror.restart()
const sync = useStoreReplaySync(existingStore, remote, {onBatch?, validateBatch?}) // same, store supplied by the app
...
`wenay-common2@2.x` exposes only Store Replay V2 directly at `api.replay`.
Store patches travel as V2 envelopes over the normal JSON RPC array wire:
there is no Store Replay negotiation, numbered batch mode, legacy line,
MessagePack, or RPB/1-RPB/3 application wire to select. `onBatch` runs after a
physical V2 envelope is applied; `validateBatch` runs before apply and may
reject it. The old React `batch` option is deprecated and ignored so source
compiled against the 1.x adapter can migrate without a flag-day rewrite.

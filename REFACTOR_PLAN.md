...
- `useOutside.tsx:73-75` (`Button`, проп `keySave`) — фича персиста статуса мертва: `saveStatus` читается, но нигде не пишется. **Либо реализовать запись, либо удалить мёртвый `keySave`** (удаление пропа — breaking).
- `RNDFunc.tsx:27` (Drag2) — дефолт ref `{ y: x, x: y }` меняет координаты местами. **Исправить на `{ x, y }`.** Может «сломать» проекты, компенсировавшие баг.
- `LeftModal.tsx:252-277` — три разных элемента пишут в один и тот же ref `viewportSize.current` → математика анимации использует «кто последний отрендерился». **Разделить refs/источник истины.**
- `LeftModal.tsx:204,237` — тернарник проверяет `viewportSize.current`, но использует `viewportWidth` (значение, не `.width`). **Привести к согласованности.**
- `LeftModal.tsx:335,432` (`LeftMenuComponent`) — проп `api(setMenu)` объявлен, но `api(...)` нигде не вызывается → весь `setMenu`-API мёртв. **Развести api** (или удалить мёртвый контракт).

### Глобальное состояние / сайд-эффекты при импорте
- `LeftModal.tsx:427` (`Modal2`) — `if (menu) setMenu(menu, key)` вызывается во время рендера (двойной сев в StrictMode). **Перенести в эффект.**
- `Modal.tsx:24` (`confirmModal`) — захардкоженный пароль `"111"` (`txt == "111"`). **Пересмотреть** (placeholder-логика в публичном API).
...
- **`chartEngine.ts` ↔ `chartEngineReact.tsx`** — две почти идентичные копии типов и `createChartEngine` с **расходящимися сигнатурами** `Panel`/`addPanel`/`resizePanel` (pixel-height vs `heightPct`, 2 vs 3 аргумента). Это два независимых публичных API. **Системный риск:** любая правка должна дублироваться в обоих или явно выбирать целевой. **План: пометить один как deprecated, свести к общему ядру в будущем major.**
